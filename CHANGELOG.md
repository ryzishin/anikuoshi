...
UI/UX fixes, watch page simplification, settings overhaul & performance — built against **APIKuoshi v2.4.0** (same live base `https://apikuoshi-v2.onrender.com`). Every change below was verified against the live API's endpoint catalog (`/api/docs.json`), the canonical shape (`src/core/shape.js`) and the art-enrichment chain (`src/core/enrich.js`); only real v2.4.0 endpoints and fields are used.

### Changed — players (§4)
- **Senshi and Plyr are removed.** Two engines remain: **Vidk** (Vidstack — direct HLS with captions, gestures and its own accessible controls) and **Embed**. The factory default is now Vidk; the Settings "Default player" preference and the under-player switcher list only these two. The `hls.js` dependency is no longer imported by the player at all (Vidstack ships its own HLS engine), cutting a large chunk off the watch-page bundle.
- **Embed sandbox removed** — the `sandbox` attribute on the embed iframe is gone entirely (several `/videojs/` provider pages refused to boot under the old allow-list). The referrer policy stays.
- **Vidk thumbnail stretching fixed** — the poster now renders letterboxed (`object-fit: contain` on the Vidstack poster element) and the watch page feeds the player the **episode thumbnail** from the v2.4.0 enrichment chain (`thumbnail` + `thumbSource`: upstream → Kitsu → TMDB → series poster) instead of always using the series poster.
- **Subtitles fixed** — tracks are proxied (CORS-safe VTT) and exactly one track is marked default using the priority: the stream's own `default` flag → an English track → the first track. Selection is available in the player's captions menu at all times.
- **Seamless switching** — server and episode switches preserve current time, volume, rate and subtitle selection where technically possible (cross-engine bridge + per-mount `startAt`). The **token freshness rule is unchanged**: every switch re-calls `/api/watch` for fresh 90 s CDN tokens before loading.
- **No rewind on toggle** — Auto-play / Auto-next / Auto-skip only flip preferences; the player instance is untouched, so toggling never resets playback to the beginning.

### Changed — watch page layout (§4)
- **The Show/Hide Servers toggle is removed** — the server panel is always visible directly below the player. The "panel hidden" card and the explanatory note under the servers are removed too.
- The toggles row that broke the layout is gone: **prev/next episode** live in the under-player strip beside the **smaller** auto-play / auto-next / auto-skip chips, with the **anime name + episode title/number (marquee for long titles)** between them — matching the spec exactly.
- **New stream loader** — the stream-resolution loader is now a moving/wiggling ellipsis (three dots waving in sequence, CSS-only, disabled under reduced motion). No circular spinner anywhere in the playback path.

### Added — unaired / no-stream handling (§4/§9)
- **Not aired yet**: an episode whose `aired` date is in the future — or a series whose status is "Not Yet Aired" — renders a clean state on the watch page with no player and no watch CTA; unaired episodes in the grid are disabled with a "not aired" chip.
- **No streams**: an episode where servers genuinely return nothing renders a clean "No stream available" empty state instead of a confusing player error.
- **Details page**: not-yet-aired shows get a disabled "Not aired yet" CTA — no watch link.

### Changed — details page (§5)
- **Complete v2.4.0 data**: the page renders every field the detail endpoints expose — synopsis, type, season, year, status, episodes, score, age rating, genres, synonyms, native title, and the full art set (poster/cover/backdrop/banner/logo). Titles honor the Anime Titles language setting.
- **Combined fetch** — meta, characters, recommendations, seasons and watch-order resolve through one `Promise.allSettled` load; the hero waits for meta so sections no longer "show incomplete then pop in".
- **The episode grid is removed** from the details page (and the redundant **"Open Player"** button with it). In its place: **Seasons / Prequel / Sequel / Specials (Movies, OVA, ONA) / Related**, grouped from `/api/meta`'s `relations` block (AniList — authoritative) plus `/api/seasons/:slug` and `/api/watch-order/:slug`, deduped by key/slug and **validated against the franchise base title so unrelated shows cannot appear**.
- **Watchlist status dropdown** — the old two-state Add-to-list button is now a dropdown: Planning, Watching, Completed, On Hold, Dropped (+ Remove from list). Persisted via `POST /api/user/list` and synced.
- **Movie distinction** — a title with exactly one episode that is a **Movie** gets a "Watch" CTA; other single-episode entries get the "Watch episode 1" equivalent.
- **Continue watching on details** — when progress exists (server record or the guest localStorage map), the primary CTA becomes "Continue EP n" with the exact saved episode.

### Changed — home page (§2)
- **Top 10 first, as a looping hero carousel** (Today/Week/Month tabs) in the style of the landing hero: full-bleed art (landscape `cover`/`backdrop` from the spotlight lane, poster fallback), big rank numeral, title + meta, arrows, dots, 6.5 s auto-advance that wraps and pauses on hover/focus.
- **Spotlight, Trending, Recently updated, Popular, Top airing, Upcoming became tabs** with a **`‹ n ›` pagination control** beside the tab bar; the feeds render as a **grid** (pagination controls density — no more horizontal rows). Pagination depth follows the API: `/api/latest-updated` is paginated; the other lanes are single-page upstream and the control shows 1/1 accordingly.

### Changed — browse page (§3)
- **The keyword filter is no longer a live search** — no suggestions, no auto-fetch. Every filter field (keyword included) commits only when **Apply** is pressed; Reset clears the form and the applied state. Multi-select genres and the "All" chips for Types/Genres are unchanged.

### Fixed — global & navigation (§1)
- **Card hrefs always use `/anime/:slug` or `/anime/:key`** — `itemKey()` now prefers the canonical id key, then the row's listing slug, and only falls back to a title when a row has neither. Title-based routing (the API-error source) is gone for v2.4.0 rows.
- **HTML entity artifacts fixed at the source** — every API response is deep-decoded in `apiFetch()` (named + numeric entities: `&amp;` → `&`, `&#039;` → `'`, …) so titles, synopses, character names and episode titles can no longer render `;amp`-style artifacts anywhere.
- **Title language honored everywhere** — the Top 10 carousel, franchise rows, relation rows, character names (via the new Characters display setting) and all remaining hardcoded spots route through `displayTitle()`; if the setting is English, every card/row/detail/watch/history surface shows `titleEnglish` (Romaji likewise).
- **Keyboard shortcuts audited** — every binding re-verified against the new engine: Space/K play-pause, ←/→ ±5 s, J/L ±10 s, ↑/↓ volume, M mute, F fullscreen, N/P episodes, 0–9 percent jumps, "/" search focus, "?" help. Fixed conflicts: Space/Enter no longer double-fire when a button/link/switch has focus, ctrl/alt/meta chords are ignored, and the removed "S" (server panel) binding is gone from the help modal.
- **Bigger header search bar** — 44 px tall on every viewport, wider resting/focus widths (focuses to 80/96 rems) and thumb-friendly, still opening the htmx poster-preview suggestions.
- **Logo links to `/home`** — not the landing page.
- **AI-generated design tells removed** — gradient buttons/chips/banners replaced with the solid accent system (`bg-primary`), the landing blur-blob and glow shadows removed, poster error placeholders neutralized, card hover calmed. The small brand marks (logo tile, loader) keep the signature gradient as the single brand accent.

### Changed — settings (§8)
- **"Default Server" → "Default Provider"** with the API's real server codenames (riyo, kaito, hana, sora, akira, yuki, miso, kenji, arashi, taiki). When the chosen provider is not available for the current anime/episode, the loader falls back to the default system (auto) ordering.
- **Title language split into two settings**: **Anime Titles** (English | Romaji) and **Characters** (English | Romaji). Characters renders Western order ("Roronoa Zoro") vs the API's family-name-first romaji ("Zoro Roronoa") — a real, observable transformation of the v2.4.0 `/api/meta/characters` data.
- **New "Default Language" setting** (Sub | Dub | System) drives the watch page's default stream selection. Legacy `defaultServer` values ("sub"/"dub") migrate automatically to Default Language.
- **Every setting is functional** — persistence (localStorage + PATCH `/api/user/preferences` profile sync) and runtime effect verified for each control; the dead-toggle audit removed nothing that isn't wired.

### Fixed — progress sync & history (§6)
- **Stale-closure flush bug**: the pagehide/unmount flush captured the first render's progress writer, so a pagehide after an episode switch flushed the position onto the WRONG episode. A latest-ref now keeps the flush pointed at the live key/episode — the exact episode and timestamp persist and resume correctly across watch page, playback, details page and `/history`. Flushes on visibilitychange, `pagehide`, pause, `ended` and 15 s milestones all verified.

### Changed — profile (§7)
- **Avatar upload accepts any image file** (`accept="image/*"`) with a WebP → JPEG → PNG encoder fallback (older Safari/Firefox can't encode canvas WebP), a 15 MB pre-check, an explicit Uploading… state and clearer error toasts.

### Performance (§1/§10)
- **Less JS on first paint**: hls.js (~500 KB) and Plyr no longer reach the client; the Vidk engine is lazy-mounted; catalog TTL caching (120 s, in-flight dedup) is kept for browse GETs.
- **Idle prefetch** of the critical routes (Home, Browse, Search, History, Settings) from the header — the sidebar links never enter the viewport, so Next's own prefetch never warmed them.
- **Faster watch page**: Vidstack loads its own HLS engine on demand; the stall watchdog now relies on engine progress events instead of polling the DOM.

### Not changed
- Stack (Next.js 16 App Router, shadcn/ui, EJS + htmx fragments, MongoDB Atlas / Prisma SQLite fallback), auth, the theme engine, the PWA, the fragments, the browse multi-genre fixes, and the API's stream fallback chain. No refactors beyond the fixes above; no invented API routes.
