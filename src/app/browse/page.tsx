...
function BrowseInner() {
    ...
    try {
      ...
      if (tab === "filter") {
        result = await filterApi({
          ...
          sort: fSort, year: fYear, epMin: fEpMin, epMax: fEpMax, page,
        });
      } else if (tab === "genre") {
        // §9: no genre selected = "All" → the whole catalog (A–Z view);
        // 1+ genres → comma-separated genre param (multi-genre API path).
        result = genreParam.length
          ? await genreList(genreParam.join(","), page)
          : await azList("all", page);
      } else if (tab === "type") {
        // §9: no type selected = "All" → the whole catalog (was: []).
  ...
  }, [tab, fKeyword, fGenre, fType, fStatus, fSeason, fLanguage, fRating, fSource, fSort, fYear, fEpMin, fEpMax, genreParam, typeParam, letter, page]);
        ...
        {([
          ...
          <button
            ...
            className={`flex min-h-[44px] items-center gap-1.5 whitespace-nowrap rounded-lg px-3.5 py-2 text-sm font-medium transition-colors ${
              tab === id ? "brand-gradient text-white" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {icon} {label}
          </button>
      ...
      {tab === "filter" && (
        <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl border border-border/70 bg-card/50 p-4 backdrop-blur sm:grid-cols-3 lg:grid-cols-6">
          ...
          <div className="col-span-2 flex items-end gap-2 sm:col-span-3 lg:col-span-2">
            <Button className="brand-gradient border-0 text-white" onClick={() => { setPage(1); load(); }}>
