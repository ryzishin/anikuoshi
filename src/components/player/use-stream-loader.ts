...
/**
 * FIX (v1.3.0 — §8): provider-aware ordering. `providerPref` is the Settings
 * "Default Provider" codename (riyo/kaito/hana/…). When the current
 * anime/episode actually HAS that provider it bubbles to the front of its
 * class; when it does NOT, the list stays in the default auto order —
 * that's the documented "fall back to default/system (auto)" behavior.
 */
export function orderStreams(streams: Stream[], pref: AudioPref, providerPref?: string): Stream[] {
  const hasProvider =
    Boolean(providerPref) &&
    providerPref !== "system" &&
    streams.some((s) => s.provider === providerPref);
  const score = (s: Stream) => {
    ...
    if (hasProvider && s.provider === providerPref) v -= 25;
    return v;
  };
...
export function useStreamLoader(opts: {
  ...
  episode: number;
  audioPref: AudioPref;
  /** v1.3.0 §8 — preferred provider codename; "system" = auto */
  providerPref?: string;
  /** bump to force a silent refresh (token refresh path) */
  refreshNonce: number;
}) {
  const { animeKey, episode, audioPref, providerPref, refreshNonce } = opts;
  ...
  const ordered = useMemo(
    () => orderStreams(streams, audioPref, providerPref),
    [streams, audioPref, providerPref]
  );
  ...
  const load = useCallback(
    async (opts2?: { silent?: boolean }) => {
      ...
      try {
        ...
        setFailure(null);
        const first = orderStreams(list, audioPref, providerPref)[0];
        setActiveIdx(0);
    ...
    [animeKey, episode, audioPref, providerPref]
  );
