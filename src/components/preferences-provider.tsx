...
import { migratePreferences } from "@/lib/store/types";
...
export function PreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  // hydrate from local storage — v1.3.0: run the defaultServer →
  // defaultLanguage/defaultProvider migration on the stored blob
  useEffect(() => {
    try {
      const raw = localStorage.getItem(KEY);
      if (raw) setPrefs(migratePreferences({ ...DEFAULTS, ...JSON.parse(raw) }));
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    setPrefs((prev) => ({
      ...
      charLang: user.preferences.charLang ?? prev.charLang,
      defaultLanguage: user.preferences.defaultLanguage ?? prev.defaultLanguage,
      defaultProvider: user.preferences.defaultProvider ?? prev.defaultProvider,
  ...
  const value = useMemo(
    () => ({
      prefs,
      titleLang: (prefs.titleLang as "romaji" | "english") || "romaji",
      charLang: (prefs.charLang as "romaji" | "english") || "romaji",
      setPref,
    }),
    [prefs, setPref]
  );
