...
cp .env.example .env            # set AUTH_SECRET; MONGODB_URI optional
npm run db:push                 # SQLite fallback path only
npm run dev                     # http://localhost:3000
```

- **Default language / provider / player / title & character language** all live in `/settings`; guests persist to localStorage, signed-in users to their profile (PATCH `/api/user/preferences`).
- The front end talks to **APIKuoshi v2.4.0** at `https://apikuoshi-v2.onrender.com` (override with `NEXT_PUBLIC_API_BASE`).
- First episode playback exercises the stream pipeline: `/api/watch` → fresh 90 s CDN tokens → Vidk (HLS) with Embed as the automatic fallback.
